# What is the address of the P2SH script wrapping the witness redeem script in this transaction
# transaction=""

RAW_TX=""



# 1) Decode the raw transaction
DECODED=$(bitcoin-cli -regtest decoderawtransaction "$RAW_TX")

# 2) Extract the ASM of the first input's scriptSig
ASM=$(echo "$DECODED" | jq -r '.vin[0].scriptSig.asm')

# 3) The redeem script is the last whitespace‑separated token
REDEEM_SCRIPT_HEX=$(echo "$ASM" | awk '{print $NF}')

# 4) Decode the redeem script to get its various addresses
DECODED_RS=$(bitcoin-cli -regtest decodescript "$REDEEM_SCRIPT_HEX")

# 5) Extract the P2SH‑wrapped‑SegWit address
P2SH_SEGWIT_ADDR=$(echo "$DECODED_RS" | jq -r '.segwit.["p2sh-segwit"]')

echo "$P2SH_SEGWIT_ADDR"